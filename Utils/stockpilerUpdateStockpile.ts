        ...
        if (await argon2.verify(password.password, body.password)) {
            ...
            if (stockpile) {
                ...
                if (JSON.stringify(newStockpileItems) !== JSON.stringify(stockpile.items)) {
                    ...
                    await collections.stockpiles.updateOne({ _id: stockpile._id }, { $set: { items: newStockpileItems, lastUpdated: currentDate } })
                }
            ...
            const stockpileUpdated = await collections.stockpiles.findOne({ name: searchQuery })
            await updateStockpileMsg(client,"",stockpileUpdated,true)
